...
The second half is memory, and the order matters. I photograph the model number first. If it already has the tech sheet for that model, we go. If it doesn't, I photograph the sheet, the folded paper inside the door or behind the kick plate, or I download it from the makers who give them out, and it gets filed by model with the fault codes, the service mode and the test points typed out. If there's no sheet to be found, it still helps, but noticeably less. The document is what makes it good. With the document in front of it, it's a tech line: it has the sheet, it has what failed on that model last time, and it never puts you on hold. Whether it beats the manufacturer's tech line is my opinion, not a measurement, and my opinion is yes.
